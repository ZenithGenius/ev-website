    ...
    class PolygonMaskInstance {
        constructor(container, engine) {
            ...
            this._parseSvgPath = async (xml, force) => {
                ...
                const { parsePaths } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require("./utils.js"))) : new Promise((resolve_1, reject_1) => { require(["./utils.js"], resolve_1, reject_1); }).then(__importStar));
            ...
            this._polygonBounce = async (particle, delta, direction) => {
                ...
                if (options.type === "inside" || options.type === "outside") {
                    ...
                    for (let i = 0, j = this.raw.length - offset; i < this.raw.length; j = i++) {
                        const pi = this.raw[i], pj = this.raw[j], { calcClosestPointOnSegment } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require("./utils.js"))) : new Promise((resolve_2, reject_2) => { require(["./utils.js"], resolve_2, reject_2); }).then(__importStar));
                        closest = calcClosestPointOnSegment(pi, pj, pos);
                        const dist = (0, engine_1.getDistances)(pos, closest);
                        [dx, dy] = [dist.dx, dist.dy];
                        if (dist.distance < radius) {
                            const { segmentBounce } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require("./utils.js"))) : new Promise((resolve_3, reject_3) => { require(["./utils.js"], resolve_3, reject_3); }).then(__importStar));
        ...
        async draw(context) {
            ...
            for (const path of this.paths) {
                ...
                if (path2d && this.offset) {
                    const { drawPolygonMaskPath } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require("./utils.js"))) : new Promise((resolve_4, reject_4) => { require(["./utils.js"], resolve_4, reject_4); }).then(__importStar));
                    drawPolygonMaskPath(context, path2d, polygonDraw.stroke, this.offset);
                }
                else if (rawData) {
                    const { drawPolygonMask } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require("./utils.js"))) : new Promise((resolve_5, reject_5) => { require(["./utils.js"], resolve_5, reject_5); }).then(__importStar));
